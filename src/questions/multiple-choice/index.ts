/**
 * multiple-choice/index.ts
 *
 * Multiple Choice Question Plugin
 *
 * Characteristics:
 * - Output answer: Sorted array of 0-indexed integers (number[]).
...
export const multipleChoicePlugin: QuestionPlugin<number[]> = {
  type: 'multiple_choice',
  label: 'Multiple Choice',
  description: 'Multiple selection mapped to an array of 0-indexed integer indices',
  ...
  component: defineComponent({
    ...
    setup(props, { emit }) {
      return () => {
        ...
        const selectedList = Array.isArray(props.value) ? props.value : [];
        const selectedSet = new Set<number>(selectedList);
  ...
  formatValue(model, value): string {
    ...
    const options = normalizeOptions(model.options);
    return value.map((idx) => options[idx]?.label ?? String(idx)).join(', ');
  },
};
